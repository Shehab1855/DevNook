...
articles_df = pd.read_sql("SELECT * FROM posts;", cnxn)


# In[3]:


#cursor.execute("SELECT * FROM evnets")

interactions_df = pd.read_sql("SELECT * FROM evnets;", cnxn)
...
users_interactions_count_df = interactions_df.groupby(['userid', 'PostId']).size().groupby('userid').size()
...
interactions_full_df = interactions_from_selected_users_df \
                    .groupby(['userid', 'PostId'])['eventStrength'].sum() \
...
def get_items_interacted(userid, interactions_df):
    ...
    interacted_items = interactions_df.loc[userid]['PostId']
...
item_popularity_df = interactions_full_df.groupby('PostId')['eventStrength'].sum().sort_values(ascending=False).reset_index()
...
class PopularityRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recommendations_df = self.popularity_df[~self.popularity_df['PostId'].isin(items_to_ignore)] \
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'PostId', 
                                                          right_on = 'PostId')[['eventStrength', 'contentId', 'title', 'url', 'lang']]
...
vectorizer = TfidfVectorizer(analyzer='word',
                             ngram_range=(1, 2),
                             min_df=0.0005,    # Adjust min_df
                             max_df=0.95,       # Adjust max_df
                             max_features=20000,  # Increase max_features
                             stop_words=stopwords_list)

item_ids = articles_df['Id'].tolist()
tfidf_matrix = vectorizer.fit_transform(articles_df['contant'])
...
# Assuming you have imported necessary libraries like tfidf_matrix, item_ids, interactions_full_df, and articles_df
...
def build_users_profile(person_id, interactions_indexed_df):
    ...
    user_item_profiles = get_item_profiles(interactions_person_df['PostId'])
...
def build_users_profiles(): 
    interactions_indexed_df = interactions_full_df[interactions_full_df['PostId'] \
...
class ContentBasedRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'PostId', 
                                                          right_on = 'PostId')[['recStrength', 'PostId']]
...
def recommend_items_for_user_for_post(topn=15, verbose=False):
    ...
    post_ids = recommendations['PostId'].tolist()
