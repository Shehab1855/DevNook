...
articles_df = pd.read_sql("SELECT * FROM projects;", cnxn)


# In[3]:


interactions_df = pd.read_sql("SELECT * FROM projectEvents;", cnxn)
...
users_interactions_count_df = interactions_df.groupby(['userid', 'projectId']).size().groupby('userid').size()
...
interactions_full_df = interactions_from_selected_users_df \
                    .groupby(['userid', 'projectId'])['eventStrength'].sum() \
...
# تكون النتيجة هي مجموعة تحتوي على معرفات المحتوى التي قام المستخدم بالتفاعل معها.
def get_items_interacted(userid, interactions_df):
    # Get the user's data and merge in the movie information.
    interacted_items = interactions_df.loc[userid]['projectId']
...
class ModelEvaluator:
    ...
    def evaluate_model_for_user(self, model, userid):
        ...
        if type(interacted_values_testset['projectId']) == pd.Series:
            person_interacted_items_testset = set(interacted_values_testset['projectId'])
        else:
            person_interacted_items_testset = set([int(interacted_values_testset['projectId'])])  
        ...
        for item_id in person_interacted_items_testset:
            ...
            valid_recs_df = person_recs_df[person_recs_df['projectId'].isin(items_to_filter_recs)]                    
            valid_recs = valid_recs_df['projectId'].values
...
item_popularity_df = interactions_full_df.groupby('projectId')['eventStrength'].sum().sort_values(ascending=False).reset_index()
...
class PopularityRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recommendations_df = self.popularity_df[~self.popularity_df['projectId'].isin(items_to_ignore)] \
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'projectId', 
                                                          right_on = 'projectId')[['eventStrength']]
...
tfidf_matrix = vectorizer.fit_transform( articles_df['Brief'])
...
def build_users_profile(person_id, interactions_indexed_df):
    ...
    user_item_profiles = get_item_profiles(interactions_person_df['projectId'])
...
def build_users_profiles(): 
    interactions_indexed_df = interactions_full_df[interactions_full_df['projectId'] \
...
class ContentBasedRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recommendations_df = pd.DataFrame(similar_items_filtered, columns=['projectId', 'recStrength']) \
                                    .head(topn)
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'projectId', 
                                                          right_on = 'projectId')[['recStrength', 'projectId']]
...
class CFRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recommendations_df = sorted_user_predictions[~sorted_user_predictions['projectId'].isin(items_to_ignore)] \
...
def recommend_items_for_user_for_project(topn=15, verbose=False):
    ...
    post_ids = recommendations['projectId'].tolist()
